...
run_test() {
  ...
  $entry -m pip install -q $TF_VERSION pytest pytest-benchmark  boto3 fastavro avro-python3 scikit-image pandas pyarrow==3.0.0 google-cloud-pubsub==2.1.0 google-cloud-bigtable==1.6.0 google-cloud-bigquery-storage==1.1.0 google-cloud-bigquery==2.3.1 google-cloud-storage==1.32.0 PyYAML==5.3.1 azure-storage-blob==12.8.1 azure-cli==2.29.0
  (cd tests && timeout -s INT 6300 $entry -m pytest  -o faulthandler_timeout=6000 --full-trace --benchmark-disable -v --import-mode=append $(find . -type f \( -iname "test_*.py" ! \( -iname "test_standalone_*.py" \) \)))
  (cd tests && timeout -s INT 6300 $entry -m pytest  -o faulthandler_timeout=6000 --full-trace --benchmark-disable -v --import-mode=append $(find . -type f \( -iname "test_standalone_*.py" \)))
}

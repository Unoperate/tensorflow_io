...
run_test() {
  ...
  (cd tests && $entry -m pytest --benchmark-disable -v --import-mode=append --forked --numprocesses=1 --dist loadfile $(find . -type f \( -iname "test_*.py" ! \( -iname "test_standalone_*.py" \) \)))

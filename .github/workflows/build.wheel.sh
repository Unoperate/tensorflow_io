...
run_test() {
  ...
  $entry -m pip install -q $TF_VERSION pytest pytest-benchmark pytest-xdist pytest-timeout boto3 fastavro avro-python3 scikit-image pandas pyarrow==3.0.0 google-cloud-pubsub==2.1.0 google-cloud-bigtable==1.6.0 google-cloud-bigquery-storage==1.1.0 google-cloud-bigquery==2.3.1 google-cloud-storage==1.32.0 PyYAML==5.3.1 azure-storage-blob==12.8.1 azure-cli==2.29.0
  (cd tests && $entry -m pytest --benchmark-disable -vv -s test_bigtable_parallel_read_rows.py)
  (cd tests && $entry -m pytest --benchmark-disable -v --timeout=1000 --import-mode=append --forked --numprocesses=auto --dist loadfile $(find . -type f \( -iname "test_*.py" ! \( -iname "test_standalone_*.py" \) \)))

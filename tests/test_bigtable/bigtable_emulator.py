...
class BigtableEmulator:
    ...
    def write_tensor(self, project_id, instance_id, table_id, tensor: tf.Tensor, rows: List[str], columns: List[str]):
        ...
        for i, row in enumerate(tensor):
            for j, value in enumerate(row):
                cmd = [cli_path, '-project', project_id, '-instance', instance_id,
                       'set', table_id, rows[i], f"{columns[j]}={value.numpy().decode()}"]
                subprocess.check_output(cmd)
